...
const LatexRenderer = React.memo(({ latex, isDisplayMath }) => {
  ...
  const staticHeightFallback = isDisplayMath ? 85 : 45;

  useEffect(() => {
    const timer = setTimeout(() => setWebViewKey(Date.now()), 200);
  ...
  const htmlContent = useMemo(() => {
    ...
    <head>
      ...
      <style>
        ...
        #math { display: inline-block; padding: 0; margin: 0; }
...
// ----------------------------------------------------------------------
// ---------- COMPONENTE: SmartMessage ---------------------------------
// ----------------------------------------------------------------------
const SmartMessage = React.memo(({ content, isDark }) => {
    ...
    <View style={{ flexShrink: 1, flexDirection: 'column' }}>
      {splitContent.filter(Boolean).map((part, index) => {
        ...
        if (isLatex) {
          ...
          if (!finalLatex.trim()) return null;
          finalLatex = finalLatex.trim().replace(/^\\n\s*|\s*\\{1,2}\s*$/g, '');
          const isDisplayMath = part.startsWith('$$');
          return <LatexRenderer key={key} latex={finalLatex} isDisplayMath={isDisplayMath} />;
        } else if (part.trim().length > 0) {
          return (
            <Text
              key={key}
              style={[styles.messageText, { color: isDark ? 'white' : '#333' }]}
            >
              {part.trim()}
            </Text>
          );
...
const ChatScreen = () => {
  ...
  const renderMessage = ({ item }) => (
    ...
    >
      {item.isUser ? (
        <Text style={[styles.messageText, { color: isDark ? 'white' : '#333' }]}>
          {item.text}
        </Text>
      ) : (
        <View style={styles.aiMessageContainer}>
          <Image
            ...
            resizeMode="contain"
          />
          <SmartMessage content={item.text} isDark={isDark} />
        </View>
      )}
    </View>
  );
  ...
  return (
    <View
      style={[
        styles.container,
        { backgroundColor: theme.background || (isDark ? '#121212' : 'white') },
      ]}
    >
      <FlatList
        ref={flatListRef}
      ...
      {loading && (
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="small" color={theme.primary || '#A1C4FC'} />
